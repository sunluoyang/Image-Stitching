...
def get_save_json_name(output_dir, image_path):
    """
    Get save image name from source image path.
    """
    if not os.path.exists(output_dir):
    ...
    image_name = os.path.split(image_path)[-1]
    name, ext = os.path.splitext(image_name)
    return os.path.join(output_dir, "{}".format(name)) + '.json'
...
def save_bbox(save_name, bbox_result):
    with open(save_name, 'w') as file_obj:
        json.dump(bbox_result, file_obj)
...
def main():
    ...
    for iter_id, data in enumerate(loader()):
        ...
        if 'bbox' in res:
            ...
            bbox_results = [bbox_results[i] for i in range(len(bbox_results)) if bbox_results[i]["score"] > thera]
        ...
        for im_id in im_ids:
            ...
            save_name = get_save_json_name(FLAGS.output_dir, image_path)
            logger.info("Detection bbox results save in {}".format(save_name))
            save_bbox(save_name, bbox_results)
